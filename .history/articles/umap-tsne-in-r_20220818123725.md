...
まず`min_dist`と`n_neighbors`それぞれについて描画したい変数の組み合わせを`expand_grid()`で生成する。`spread`は今回は`2`で固定した^[上記スライドではspreadが1になっているが、min_distよりも小さい値を設定できないはずなので、min_dist = 5は誤植かなと思う]。
```r
