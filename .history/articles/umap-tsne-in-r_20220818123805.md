...
まず`min_dist`と`n_neighbors`それぞれについて描画したい変数の組み合わせを`expand_grid()`で生成する。`spread`は今回は`2`で固定した^[上記スライドでは`spread`が1になっているが、`min_dist`よりも小さい値を設定できないはずなので、`min_dist = 5`は誤植かなと思う]。
```r
